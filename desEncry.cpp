...
static void IP(const int input[64],int output[64],int table[64]);//初始IP置换，64->64,左右分别32
static void E(const int input[32],int output[48],int table[48]);//E扩展32->48
...
static void PC_1(const int input[64],int output[56],int table[56]);//PC_1，用于密钥置换
...
static void F_func(int input[32],int output[32], int subkey[48]);//完成DES算法轮变换,就是f函数的内部实现，就是一个异或而已
...
static void  DES_Efun(char input[8],char key_in[8],int output[64]);//加密实现
static void  DES_Dfun(int input[64],char key_in[8],char output[8]);//解密实现
...
int main(int argc,char *argv[]){
    ...
    char MI[9]={0};
    //printf("请输入明文\n");
    strcpy(inputmi,argv[2]);
    //CharToBit(argv[2],inputmi,64);
    /*for(int h=0;h<NUM;h++)
    {
        printf("%d,",inputmi[h]);
    }*/
    ...
    else
    {
        ...
        for(m=0;m<grpnum;m++)
        {
            ...
            {
                //printf("ok1");
                //printf("%c",inputmi[m*64+n]);
                if(inputmi[m*64+n]!=0)
                {
                    //printf("%c",inputmi[m*64+n]);
                    grp[m][n]=inputmi[m*64+n];
                    //printf("%d:%c,",n,grp[m][n]);
                }
            }
        }
        /*for(m=0;m<grpnum;m++)
